...
class Data:
    ...
    def data_preprocess(self):
        ...
        self.origin_location = self.city_location.copy()
        # 去掉销量为0的列
        ...
        try:
            ...
            result = {
                "success": False,
                "code": "500",
                "message": '请检查Regional hubs（Train)数据格式和列名  "城市", "固定费用", "是否启用"'
            }
            print(json.dumps(result))
            return False
        ...
        try:
            ...
            result = {
                "success": False,
                "code": "500",
                "message": '请检查Regional hubs（Ship)数据格式和列名  "城市", "固定费用", "是否启用"'
            }
            print(json.dumps(result))
            return False
        ...
        try:
            ...
            result = {
                "success": False,
                "code": "500",
                "message": '请检查Province-hub数据格式和列名  "铁路1", "铁路2", "铁路3", "铁路4", "铁路5", "铁路6", "铁路7", "铁路8" "水路1", "水路2", "水路3", "水路4"'
            }
            print(json.dumps(result))
            return False
        return True
    ...
    def calculate_distance_matrix(self):
        cities = self.origin_location.index.tolist()
        ...
        for i in range(len(cities)):
            for j in range(i, len(cities)):
                if i == j:
                    ...
                    lat1, lon1 = self.origin_location.loc[cities[i], ["纬度", "经度"]]
                    lat2, lon2 = self.origin_location.loc[cities[j], ["纬度", "经度"]]
